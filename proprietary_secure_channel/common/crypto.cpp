...
Crypto::Crypto() { m_initialized = init_mbedtls(); }
...
bool Crypto::init_mbedtls(void) {
  ...
  // Initialize entropy.
  res = mbedtls_ctr_drbg_seed(&m_ctr_drbg_contex, mbedtls_entropy_func,
                              &m_entropy_context, NULL, 0);
  if (res != 0) {
  ...
  res =
      mbedtls_rsa_gen_key(mbedtls_pk_rsa(m_pk_context), mbedtls_ctr_drbg_random,
  ...
  res = mbedtls_pk_write_pubkey_pem(&m_pk_context, m_public_key,
                                    sizeof(m_public_key));
  if (res != 0) {
 ...
 */
bool Crypto::encrypt(const uint8_t *pem_public_key, const uint8_t *data,
                     size_t data_size, uint8_t *encrypted_data,
  ...
  // Encrypt the data.
  res = mbedtls_rsa_pkcs1_encrypt(rsa_context, mbedtls_ctr_drbg_random,
                                  &m_ctr_drbg_contex, MBEDTLS_RSA_PUBLIC,
                                  data_size, data, encrypted_data);
  if (res != 0) {
 ...
 */
bool Crypto::decrypt(const uint8_t *encrypted_data, size_t encrypted_data_size,
  ...
  res = mbedtls_rsa_pkcs1_decrypt(
      rsa_context, mbedtls_ctr_drbg_random, &m_ctr_drbg_contex,
      MBEDTLS_RSA_PRIVATE, &output_size, encrypted_data, data, output_size);
  if (res != 0) {
...
bool Crypto::get_rsa_modulus_from_pem(const char *pem_data, size_t pem_size,
  ...
  mbedtls_pk_init(&ctx);
  res = mbedtls_pk_parse_public_key(&ctx, (const unsigned char *)pem_data,
                                    pem_size);
  if (res != 0) {
  ...
  if (modulus_local == NULL) {
    TRACE_ENCLAVE("malloc for modulus failed with size %zu:\n",
                  modulus_local_size);
    goto exit;
  }

  res = mbedtls_rsa_export_raw(rsa_ctx, modulus_local, modulus_local_size, NULL,
 ...
 */
bool Crypto::encrypt_gcm(const uint8_t *sym_key, uint32_t sequence_num,
  ...
  res = mbedtls_gcm_crypt_and_tag(&gcm_context, MBEDTLS_GCM_ENCRYPT, data_size,
                                  iv_str, IV_SIZE, add_str, add_len, data,
                                  encrypted_data, tag_len, tag_output);

  if (res != 0) {
    TRACE_ENCLAVE("encrypt_gcm: mbedtls_gcm_crypt_and_tag failed with %d\n",
                  res);
    goto exit;
  }
 ...
 */
bool Crypto::decrypt_gcm(const uint8_t *sym_key, const uint8_t *iv_str,
                         const uint8_t *add_str, const uint8_t *encrypted_data,
                         size_t encrypted_data_size, const uint8_t *tag_str,
                         uint8_t *data, size_t *data_size) {
  mbedtls_gcm_context gcm_context;
  bool result = false;
  int res = 0;
  ...
  res = mbedtls_gcm_auth_decrypt(&gcm_context, *data_size, iv_str, IV_SIZE,
                                 add_str, ADD_SIZE, tag_str, tag_len,
                                 encrypted_data, data);
  if (res != 0) {
    TRACE_ENCLAVE("Decrypt_gcm: mbedtls_gcm_auth_decrypt failed with %d\n",
                  res);
    goto exit;
  }
...
int Crypto::sign(const unsigned char *hash_data, size_t hash_size,
  ...
  rc = mbedtls_rsa_pkcs1_sign(rsa_context, mbedtls_ctr_drbg_random,
                              &m_ctr_drbg_contex, MBEDTLS_RSA_PRIVATE,
                              MBEDTLS_MD_SHA256, hash_size, hash_data, sig);

exit:
  return rc;
}

int Crypto::verify_sign(const uint8_t *pem_public_key,
                        const unsigned char *hash_data, size_t hash_size,
  ...
  rc = mbedtls_rsa_pkcs1_verify(rsa_context, mbedtls_ctr_drbg_random,
                                &m_ctr_drbg_contex, MBEDTLS_RSA_PUBLIC,
                                MBEDTLS_MD_SHA256, hash_size, hash_data, sig);
exit:
  mbedtls_pk_free(&key);
