 ...
 */
bool Attestation::generate_remote_report(const uint8_t *data,
  ...
  // function.
  result = oe_get_report(OE_REPORT_FLAGS_REMOTE_ATTESTATION,
                         sha256, // Store sha256 in report_data field
                         sizeof(sha256),
                         NULL, // opt_params must be null
                         0, &temp_buf, remote_report_buf_size);
  if (result != OE_OK) {
 ...
 * report_data field.
 */
bool Attestation::attest_remote_report(const uint8_t *remote_report,
  ...
  if (memcmp(parsed_report.identity.signer_id, m_enclave_mrsigner, 32) != 0) {
    ...
    TRACE_ENCLAVE("identity.signer_id %s", parsed_report.identity.signer_id);

    for (int i = 0; i < 32; i++) {
      TRACE_ENCLAVE("m_enclave_mrsigner[%d]=0x%0x\n", i,
    ...
    for (int i = 0; i < 32; i++) {
      TRACE_ENCLAVE("parsedReport.identity.signer_id)[%d]=0x%0x\n", i,
