...
ecall_dispatcher::ecall_dispatcher(const char *name,
...
bool ecall_dispatcher::initialize(const char *name) {
  ...
  if (!m_crypto->get_rsa_modulus_from_pem(
          ...
          m_enclave_config->other_enclave_pubkey_pem_size, &modulus,
          &modulus_size)) {
    goto exit;
  }
...
int ecall_dispatcher::get_remote_report_with_pubkey(
    uint8_t **pem_key, size_t *key_size, uint8_t **remote_report,
...
int ecall_dispatcher::verify_report_and_set_pubkey(uint8_t *pem_key,
                                                   size_t key_size,
  ...
  if (m_attestation->attest_remote_report(remote_report, remote_report_size,
                                          pem_key, key_size)) {
...
int ecall_dispatcher::establish_secure_channel(uint8_t **key,
                                               size_t *key_size) {
  ...
  if (m_crypto->encrypt(m_crypto->get_the_other_enclave_public_key(),
                        m_enclave_config->sym_key, DIGEST_SIZE,
                        encrypted_key_buf, &encrypted_key_size))

  {
    ...
    if (m_crypto->sign(digest, sizeof(digest), signature, &signature_size) !=
        0) {
      goto exit;
    }

    TRACE_ENCLAVE("enclave: establish_secure_channel: signature_size = %ld",
                  signature_size);
    ...
    TRACE_ENCLAVE("enclave: establish_secure_channel: total_size = %ld",
                  total_size);
    *key = host_buf;
    *key_size = total_size;
  } else {
 ...
 */
int ecall_dispatcher::acknowledge_secure_channel(uint8_t *encrypted_key_buf,
  ...
  rc = m_crypto->verify_sign(m_crypto->get_the_other_enclave_public_key(),
                             digest, DIGEST_SIZE, signature, signature_size);
  if (rc != 0) {
  ...
  if (m_crypto->decrypt(encrypted_key_buf, ENCRYPTED_KEY_SIZE, data,
                        &data_size)) {
    TRACE_ENCLAVE(
...
int ecall_dispatcher::generate_encrypted_message(uint8_t **data, size_t *size) {
  ...
  if (m_channel_state != SECURE_CHANNEL_STATE) {
    TRACE_ENCLAVE("ecall_dispatcher failed as Secure Channel isn't available.");
    goto exit;
  }
  ...
  if (m_crypto->encrypt_gcm(
          m_enclave_config->sym_key, ++(m_enclave_config->sequence_number),
          m_enclave_config->enclave_secret_data, ENCLAVE_SECRET_DATA_SIZE,
          iv_str, encrypted_data_buf, &encrypted_data_size, tag_str)) {
    ...
    *size += ENCLAVE_SECRET_DATA_SIZE;
    ...
    TRACE_ENCLAVE(
        ...
        "total_size=%ld",
        encrypted_data_size, total_size);
    *data = host_buf;
  } else {
    TRACE_ENCLAVE("enclave: generate_encrypted_message: encrypt_gcm failed\n");
    goto exit;
  }
  ret = 0;
exit:
...
int ecall_dispatcher::process_encrypted_msg(uint8_t *encrypted_data,
                                            size_t encrypted_data_size) {
  ...
  memcpy(tag_str, &encrypted_data[ENCLAVE_SECRET_DATA_SIZE + IV_SIZE],
  ...
  if (m_crypto->decrypt_gcm(m_enclave_config->sym_key, iv_str, add_str,
                            encrypted_data, ENCLAVE_SECRET_DATA_SIZE, tag_str,
                            data, &data_size)) {
    ...
    for (uint32_t i = 0; i < ENCLAVE_SECRET_DATA_SIZE; ++i) {
      ...
      if (m_enclave_config->enclave_secret_data[i] != data[i]) {
        printf("Expecting [0x%x] but received unexpected value "
               "[0x%x]\n ",
               m_enclave_config->enclave_secret_data[i], data[i]);
        ret = 1;
        break;
      }
    }
