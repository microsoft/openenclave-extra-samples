...
#include "proprietarysecurechannel_u.h"
...
oe_enclave_t *create_enclave(const char *enclave_path) {
  ...
  oe_result_t result = oe_create_proprietarysecurechannel_enclave(
      enclave_path, OE_ENCLAVE_TYPE_SGX, OE_ENCLAVE_FLAG_DEBUG, NULL, 0,
      &enclave);

  if (result != OE_OK) {
    printf("Host: oe_create_proprietarysecurechannel_enclave failed. %s",
...
int main(int argc, const char *argv[]) {
  ...
  printf("Host: requesting a remote report and the encryption key from 1st "
         "enclave\n");
  result =
      get_remote_report_with_pubkey(enclave_a, &ret, &pem_key, &pem_key_size,
                                    &remote_report, &remote_report_size);
  if ((result != OE_OK) || (ret != 0)) {
    printf("Host: verify_report_and_set_pubkey failed. %s",
           oe_result_str(result));
    if (ret == 0)
      ret = 1;
    goto exit;
  ...
  result = verify_report_and_set_pubkey(enclave_b, &ret, pem_key, pem_key_size,
                                        remote_report, remote_report_size);
  if ((result != OE_OK) || (ret != 0)) {
    printf("Host: verify_report_and_set_pubkey failed. %s",
           oe_result_str(result));
  ...
  printf("Host: Requesting a remote report and the encryption key from "
         "2nd enclave=====\n");
  result =
      get_remote_report_with_pubkey(enclave_b, &ret, &pem_key, &pem_key_size,
                                    &remote_report, &remote_report_size);
  if ((result != OE_OK) || (ret != 0)) {
    printf("Host: verify_report_and_set_pubkey failed. %s",
           oe_result_str(result));
  ...
  result = verify_report_and_set_pubkey(enclave_a, &ret, pem_key, pem_key_size,
                                        remote_report, remote_report_size);
  if ((result != OE_OK) || (ret != 0)) {
    printf("Host: verify_report_and_set_pubkey failed. %s",
           oe_result_str(result));
  ...
  result = establish_secure_channel(enclave_a, &ret, &encrypted_key,
                                    &encrypted_key_size);
  if ((result != OE_OK) || (ret != 0)) {
    printf("Host: establish_secure_channel failed. %s", oe_result_str(result));
    if (ret == 0)
      ret = 1;
    goto exit;
  }

  printf("Received encrypted data to establish secure channel of size = %ld\n",
         encrypted_key_size);

  printf("Host: Requesting 2nd enclave to acknowledge secure channel\n");
  result = acknowledge_secure_channel(enclave_b, &ret, encrypted_key,
                                      encrypted_key_size);
  if ((result != OE_OK) || (ret != 0)) {
    printf("Host: acknowledge_secure_channel failed. %s\n",
           oe_result_str(result));
    if (ret == 0)
      ret = 1;
    goto exit;
  ...
  result = generate_encrypted_message(enclave_a, &ret, &encrypted_msg,
                                      &encrypted_msg_size);
  if ((result != OE_OK) || (ret != 0)) {
    printf("Host: generate_encrypted_message failed. %s\n",
           oe_result_str(result));
    if (ret == 0)
      ret = 1;
    goto exit;
  ...
  if ((result != OE_OK) || (ret != 0)) {
    printf("Host: process_encrypted_msg failed. %s\n", oe_result_str(result));
    if (ret == 0)
      ret = 1;
    goto exit;
  }
  ...
  result = generate_encrypted_message(enclave_b, &ret, &encrypted_msg,
                                      &encrypted_msg_size);
  if ((result != OE_OK) || (ret != 0)) {
    printf("Host: generate_encrypted_message failed. %s\n",
           oe_result_str(result));
  ...
  result =
      process_encrypted_msg(enclave_a, &ret, encrypted_msg, encrypted_msg_size);
  if ((result != OE_OK) || (ret != 0)) {
    printf("host process_encrypted_msg failed. %s\n", oe_result_str(result));
    if (ret == 0)
      ret = 1;
